...
class DrawingList extends Component {
    ...
    render() {
        const drawings = this.state.drawings.map(drawing => (
            <li className="DrawingList-item" style={{color: 'black'}} key={drawing.id} onClick={event => this.props.selectDrawing(drawing)}>{drawing.name}</li>
        ));
        return (
                <ul className="DrawingList">{drawings}</ul>
        )
    }
}
